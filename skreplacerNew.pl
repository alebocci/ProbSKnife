...
query(test(smallExample2,[(charact2,low)],Nsw)).

test(AppId,NewLabels,NewTypedSw):-
    application(AppId, _, Software),
    softwareLabel(NewLabels,Software,NewTypedSw),
    softwareOk(NewLabels,NewTypedSw).

%query(test2([(data1,low)],[data2])).
test2(NewLabels,Data):- labelsInList(NewLabels,Data).
...
impactedHWOk(NewLabels, Hardware):-
    impactedHW(NewLabels,Hardware,ImpactedHW),
    hardwareOK(NewLabels, ImpactedHW).

impactedHW(NewLabels, [HW|HWs],[HW|ImpactedHWs]):-
    hardware(Hw, Data,Characteristics,_),
    labelsInList(NewLabels,Data),
    impactedHW(NewLabels,HWs,ImpactedHWs).
impactedHW(NewLabels, [HW|HWs],[HW|ImpactedHWs]):-
    hardware(Hw, Data,Characteristics,_),
    \+labelsInList(NewLabels,Data),
    labelsInList(NewLabels,Characteristics),
    impactedHW(NewLabels,HWs,ImpactedHWs).
impactedHW(NewLabels, [HW|HWs],ImpactedHWs):-
    hardware(Hw, Data,Characteristics,_),
    \+labelsInList(NewLabels,Data),
    \+labelsInList(NewLabels,Characteristics),
    impactedHW(NewLabels,HWs,ImpactedHWs).
...
labelsInList([(L,_)|Ls],List):-
    labelInList(List,L).
labelsInList([(L,_)|Ls],List):-
    \+labelInList(List,L),
    labelsInList(Ls,List).
%labelsInList([],_).
...
softwareLabel(NewTags,[Sw|Sws],[(Sw,TData,CharLabel)|LabelledSws]):-
    software(Sw, Data,Characteristics,_,_),
    labelSw(NewTags, Data, Characteristics,(TData,TChar)),
    charLabel(TData,TChar,CharLabel),%put safe if TData <= TChar
    softwareLabel(NewTags,Sws,LabelledSws).
softwareLabel(_,[],[]).
%single software labelling
softwareLabel(NewTags,Sw,(Sw,TData,CharLabel)):-
    software(Sw, Data,Characteristics,_,_),
    labelSw(NewTags, Data, Characteristics,(TData,TChar)),
    charLabel(TData,TChar,CharLabel).%put safe if TData <= TChar

charLabel(TData,TChar,safe):- gte(TChar,TData).
charLabel(TData,TChar,TChar):- lt(TChar,TData).
